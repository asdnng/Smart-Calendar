...
public class JWTUtil {
    ...
    static {
        String secretKeySTring = "mungyeongmungyeongmungyeongmungy"; //32bytes
        secretKey = new SecretKeySpec(secretKeySTring.getBytes(StandardCharsets.UTF_8), Jwts.SIG.HS256.key().build().getAlgorithm());

        accessTokenExpiresIn = 3600L * 1000; // 1hrs
        refreshTokenExpiresIn = 604800L * 1000; // seven days
    }
    ...
    //JWT (access/refresh) token creation method
